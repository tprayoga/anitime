...
import DiagnosisModal from 'src/components/modal-domba/dokter-hewan/diagnosis-modal';
import DiagnosisModal2 from 'src/components/modal-domba/dokter-hewan/diagnosis2-modal';
import SurveilansPenyakitModal from 'src/components/modal-domba/dokter-hewan/surveilans-penyakit-modal';
import PemeriksaanLengkapModal from 'src/components/modal-domba/dokter-hewan/pemeriksaan-lengkap-modal';
...
export default function LaporanSurveilansDetailView({ id }) {
  ...
  const {
    data: dataSurveilansTernak,
    error,
    loading: loadingSurveilansTernak,
    getOneData: getSurveilans,
  } = useGetOneData();
  const { data: dataSurveilansDokter, getFirstListItem: getSurveilansDokter } =
    useGetFirstListItem();
  const { data: dataPemeriksaanLengkap, getFirstListItem: getPemeriksaanLengkap } =
    useGetFirstListItem();
  ...
  const statusOptions = [
    ...
    {
      value: 'recordAnamnesis',
      label: 'Record Anamnesis',
      disabled: dataPemeriksaanLengkap || !dataSurveilansDokter ? true : false,
    },
    {
      value: 'diagnosis',
      label: 'Diagnosis',
        ...
        <Grid container spacing={3}>
          <Grid xs={12}>
            <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}>
              <AnitimeBreadcrumbs
                links={[
                  { name: 'Laporan Surveilans', href: paths.dombaDokterHewan.surveilans.root },
                  { name: `${dataSurveilansTernak?.expand?.ternak?.noFID}` },
                ]}
              />
              ...
              <Grid container spacing={3} px={4} py={2}>
                <Grid item xs={6}>
                  ...
                  <Typography variant="body2" fontWeight={'bold'}>
                    {dataSurveilansTernak?.expand?.ternak?.noFID}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="body2">Tanggal Laporan</Typography>
                  <Typography variant="body2" fontWeight={'bold'}>
                    {fDate(dataSurveilansTernak?.created)}
                  </Typography>
                </Grid>
                <Grid item xs={12}>
                  ...
                  <Typography variant="body2" fontWeight={'bold'}>
                    {fDate(dataSurveilansTernak?.perkiraanWaktu)}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  ...
                  >
                    {dataSurveilansTernak?.expand?.createdBy?.name}
                  </Typography>
                </Grid>
                <Grid item xs={15}>
              ...
              <Card sx={{ height: '100%' }}>
                ...
                <Grid container spacing={3} px={4} py={2}>
                  <Grid item xs={6}>
                    <Typography variant="body2">No FID Ternak</Typography>
                    <Typography variant="body2" fontWeight={'bold'}>
                      {dataSurveilansDokter?.expand?.ternak?.noFID}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2">Tanggal Laporan</Typography>
                    <Typography variant="body2" fontWeight={'bold'}>
                      {fDate(dataSurveilansDokter?.created)}
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Typography variant="body2">
                    ...
                    <Typography variant="body2" fontWeight={'bold'}>
                      {fDate(dataSurveilansDokter?.perkiraanWaktu)}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography variant="body2">Petugas</Typography>
                    <Typography
                      ...
                      sx={{ textTransform: 'capitalize' }}
                    >
                      {dataSurveilansDokter?.expand?.createdBy?.name}
                    </Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <Typography variant="body2">Gejala</Typography>
                  ...
                  <Grid item xs={12}>
                    <Typography variant="body2">{`Foto Gejala : ${
                      DOKTER_GEJALA_IMAGE_URL.length === 0 ? ' - ' : ''
                    }`}</Typography>
                    <Stack flexDirection={'row'} spacing={1} flexWrap={'wrap'} mt={1}>
                  ...
                  <Grid item xs={12}>
                    <Typography variant="body2">{`Jenis Sample : ${
                      DOKTER_SAMPLE_IMAGE_URL.length === 0 ? ' - ' : ''
                    }`}</Typography>
                    <Stack flexDirection={'row'} spacing={1} flexWrap={'wrap'} mt={1}>
