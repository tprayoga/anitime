...
import { useEditUser, useCreateUser } from 'src/api/peternakan/manage-user';
...
export default function ManageNewEditForm({ currentUser }) {
  ...
  const { createData: createDataUser } = useCreateData();
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    //     router.push(paths.peternakan.manageUser.root);
    //   } catch (error) {
    //     console.log(error);
    //     enqueueSnackbar(
    //       error?.email
    ...
    const body = {
      ...data,
      role: 'peternakan',
      nik: generateRandomNumber(),
      nib: generateRandomNumber(),
    };

    try {
      await createDataUser(body, 'users');
      router.push(paths.dombaIntiPeternakan.plasma.root);
      ...
      <Grid xs={12} md={8}>
        <Card>
          ...
          <Stack spacing={3} sx={{ p: 3 }}>
            ...
            />
            <RHFTextField name="alamat" label="Alamat" multiline rows={4} />
          </Stack>
        </Card>
      </Grid>
    </>
