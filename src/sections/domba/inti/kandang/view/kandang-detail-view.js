...
export default function KandangDetailView({ id }) {
  ...
  const options = () => {
    return {
      filter: `kandang.peternakan = "${user.id}" && kandang.id = "${id}" ${
        ...
        dataPen.map(async (prev, index) => {
          ...
          try {
            const resJantan = await getTernak(
              1,
              5,
              `pen.kandang.peternakan = "${user.id}" && pen.kandang.id = "${id}" && pen.id = "${prev?.id}" && jenisKelamin = "Jantan"`,
              '-created',
              'ternak'
            );
            const resBetina = await getTernak(
              1,
              5,
              `pen.kandang.peternakan = "${user.id}" && pen.kandang.id = "${id}" && pen.id = "${prev?.id}"  && jenisKelamin = "Betina"`,
              '-created',
              'ternak'
            );
      ...
      <Grid container spacing={3}>
        <Box sx={{ px: 2, py: 2 }}>
          <Button
            ...
            href={paths.dombaPeternakan.kandang.root}
