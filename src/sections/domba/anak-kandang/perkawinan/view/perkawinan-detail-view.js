...
import { Button, Card, MenuItem, Tooltip, Typography } from '@mui/material';
...
const statusOptions = [
  ...
  { value: 'rekamKelahiran', label: 'Rekam Kelahiran' },
];
        ...
        <Grid xs={12}>
          <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}>
            <Button
              ...
              onClick={() => {
                router.push(`${paths.dombaAnakKandang.perkawinan.root}`);
              }}
            >
              Back
            </Button>
      ...
      <CustomPopover open={popover.open} onClose={popover.onClose} arrow="top-right">
        {statusOptions.map((option) => (
          <MenuItem
            key={option.value}
            onClick={() => {
              if (option.value === 'rekamKelahiran') openModal1.onTrue();
              popover.onClose();
              // setType('CREATE');
