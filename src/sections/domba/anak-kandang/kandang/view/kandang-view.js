...
import AddDataKandangModal from 'src/sections/domba/kandang/addDataKandang';
import { AddKandangModal } from 'src/components/modal-domba/anak-kandang';
...
export default function KandangView() {
        ...
        <Typography
          ...
          onClick={() => {
            user.role === 'anakKandang'
              ...
              : window.open(
                  `${paths.dombaIntiAnakKandang.kandang.detail(params?.row?.id)}`,
                  '_blank'
                );
          }}
        >
  ...
  const options = () => {
    return {
      filter: `peternakan = "${user.createdBy}" ${
  ...
  const optionPen = () => {
    return {
      filter: `kandang.peternakan = "${user.createdBy}" && kandang.id = "${penGrid?.id}" ${
        dataGridTable?.filter || debouncedSearch
  ...
  const optionTernak = () => {
    return {
      filter: `pen.kandang.peternakan = "${user?.createdBy}" && pen.id = "${ternakGrid?.id}" ${
        dataGridTable?.filter || debouncedSearch
        ...
        dataKandang.map(async (prev, index) => {
          ...
          try {
            const resJantan = await getTernak(
              1,
              5,
              `pen.kandang.peternakan = "${user.createdBy}" && pen.kandang.id = "${prev.id}" && jenisKelamin = "Jantan"`,
              '-created',
              'ternak'
            );
            const resBetina = await getTernak(
              1,
              5,
              `pen.kandang.peternakan = "${user.createdBy}" && pen.kandang.id = "${prev.id}" && jenisKelamin = "Betina"`,
              '-created',
              'ternak'
            );
        ...
        dataPen.map(async (prev, index) => {
          ...
          try {
            const resJantan = await getTernak(
              1,
              5,
              `pen.kandang.peternakan = "${user.createdBy}" && pen.id = "${prev.id}" && jenisKelamin = "Jantan"`,
              '-created',
              'ternak'
            );
            const resBetina = await getTernak(
    ...
    <>
        ...
        <Grid container spacing={3}>
          <Grid xs={12} md={12}>
            <Card>
              ...
              <Stack display={'flex'} justifyContent={'end'} alignItems={'end'} width={'100%'}>
                <ToggleButtonGroup
                  size="small"
                  value={view}
                  exclusive
                    ...
                    <Stack sx={{ mx: 3 }}>
                      <Breadcrumbs>
                        <Typography
                          sx={{
                            color:
                              !penGrid?.id && !ternakGrid.id ? 'text.primary' : 'text.disabled',
                            cursor: 'pointer',
                          }}
                          onClick={() => {
                            setPenGrid({});
      ...
      {openModal.value && (
        <AddKandangModal
          ...
          type={type}
          setType={setType}
          refetch={() => getKandang(1, 10, options()?.filter, options()?.sort, 'kandang')}
        />
      )}
    </>
  );
