...
export default function TableAnitime({
  ...
  const [statusOption, setStatusOption] = useState(statusOptions[0]?.filter || '');
  ...
  useEffect(() => {
    if (collectionId) {
      ...
      if (statusOption.length) {
        ...
        if (debounceFilter) {
          filter = `${statusOption} && ${keys[0]} ~ "${debounceFilter}"`;
        } else {
          // filter = `${key} = "${value}"`;
          filter = statusOption;
        }
                ...
                {loading
                  ...
                  : data.map((row, index) => (
                      <TableRows
                        ...
                        disabledDelete={disabledDelete}
                        //
                        collapseDisabled={collapseDisabled && collapseDisabled(row)}
                        popoverDisabled={popoverDisabled && popoverDisabled(row)}
                        //
                        propsCollapse={propsCollapse}
                      />
                    ))}
