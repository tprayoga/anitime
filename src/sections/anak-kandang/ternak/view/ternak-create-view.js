...
export default function TernakCreateView() {
    ...
    const {
        data: dataJenisBreed,
        error: errorJenisBreed,
        loading: loadingJenisBreed,
        getFullData: getJenisBreed
    } = useGetFulLData();

    const {
        data: dataJenisHewan,
        error: errorJenisHewan,
        loading: loadingJenisHewan,
        getFullData: getJenisHewan
    } = useGetFulLData();

    const {
        data: dataJenisKelamin,
        error: errorJenisKelamin,
        loading: loadingJenisKelamin,
        getFullData: getJenisKelamin
    } = useGetFulLData();
    ...
    const {
        ...
        getFullData: getKandang
    } = useGetFulLData();

    const { createData: createTernak, error: errorTernak, loading: loadingTernak } = useCreateData();
    const { updateData: updateKandang, error: errorUpdateKandang, loading: loadingUpdateKandang } = useUpdateData();


    useEffect(() => {

        getJenisHewan("listJenisHewan");
        getJenisKelamin("listJenisKelamin");
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        const selectedKandang = dataKandang?.find((element) => {
            return element.ternak.length < element.limitKandang && element.peternakan === user.user.createdBy;
        });

        if(!selectedKandang){
            enqueueSnackbar('Tidak ada Kandang yang tersedia', { variant: 'error' });
            return;
        }
        ...
        try {
            const responseData = await createTernak(updatedDataTernak, "ternak");
