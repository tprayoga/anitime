...
export default function DashboardPeternakanView() {
  ...
  const filterGrid = (id) => {
    let data;
    if (windowSize < 996) {
      if (getFromLS('dashboard', 'layout')) {
        data = getFromLS('dashboard', 'layout').sm.filter((item) => item.i === id)[0];
      } else {
        data = fullLayout?.sm?.filter((item) => item.i === id)[0];
      }
    } else if (windowSize > 996 && windowSize < 1200) {
      if (getFromLS('dashboard', 'layout')) {
        data = getFromLS('dashboard', 'layout').md.filter((item) => item.i === id)[0];
      } else {
        data = fullLayout?.md?.filter((item) => item.i === id)[0];
      }
    } else if (windowSize > 1200) {
      if (getFromLS('dashboard', 'layout')) {
        data = getFromLS('dashboard', 'layout').lg.filter((item) => item.i === id)[0];
      } else {
          ...
          <Stack spacing={2} direction={'row'} alignItems="center" justifyContent="space-between">
            ...
            <Stack spacing={2} direction={'row'}>
              ...
              <Button
                ...
                onClick={() => {
                  if (editState) saveToLS('dashboard', 'layout', dashboardLayout);
          ...
          >
            <Box>
              <ResponsiveGridLayout
                ...
                // resizeHandles={['se']}
                layouts={
                  getFromLS('dashboard', 'layout') ? getFromLS('dashboard', 'layout') : fullLayout
                }
