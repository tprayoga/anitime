...
export function useGetData() {
  ...
  const getData = async (
    ...
    try {
      setLoading(true);
      const data = await pb.collection(collectionName).getList(page, perPage, {
        filter: filter,
        sort: sort,
        expand: expand,
      });
...
export function useGetOneData() {
  ...
  const getOneData = async (id, collectionName, expand, filter) => {
    try {
      setLoading(true);
      const response = await pb.collection(collectionName).getOne(id, {
        expand: expand,
        filter: filter,
      });
...
export function useGetFirstListItem() {
  ...
  const getFirstListItem = async (collectionName, filter, expand) => {
    try {
      setLoading(true);
      const response = await pb.collection(collectionName).getFirstListItem(filter, {
        expand: expand,
      });
...
export function useDeleteData() {
  const deleteData = async (id, collectionName) => {
    try {
      const response = await pb.collection(collectionName).delete(id);
