  ...
  const [IMAGE_URL, SET_IMAGE_URL] = useState(
    type === 'EDIT'
      ? dataSurveilans[0]?.fotoGejala.map((data) => {
          return `${FILES_API}/${dataSurveilans[0]?.collectionId}/${dataSurveilans[0]?.id}/${data}?token=${pbDomba.authStore.token}`;
        })
      : ''
  );
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    if (type === 'CREATE') {
      try {
        ...
        // update status surveilans menjadi evaluasi
        const updateSurveilans = {
          ...dataSurveilansTernak,
          status: 'Evaluasi',
        };
        await updateData(dataSurveilansTernak.id, updateSurveilans, 'surveilansTernak');
        ...
        const dataTernak = dataSurveilansTernak.expand.ternak;
        const ternakBody = {
          ...dataTernak,
