...
export default function AnamnesisModal({
  ...
  const schema = Yup.object().shape({
    ...
    // warna: Yup.string().required('Perkiraan Waktu Gejala is required'),
  ...
  useEffect(() => {
    if (dataSurveilans) {
      setValue('rfid', dataSurveilans?.expand?.ternak?.RFID);
      ...
      setValue('warna', dataSurveilans?.expand?.ternak?.warna);
