...
export default function PemeriksaanLengkapModal({ open, onClose, refetch, setRefetch, dataSurveilans, ...other }) {

    // create back button (done)
    // create current page : 1/4, 9/12
    // tampung data global
    // handle button jadi submit waktu di last form

    const {
        createData: createPemeriksaanLengkap,
        loading: loading,
    } = useCreateData();
    ...
    const handleNext = async (data) => {
        ...
        if (findIndex !== formList.length - 1) {
            ...
            const body = {
                ...
                surveilans: dataSurveilans ? dataSurveilans.id : '',
    ...
    const isLastForm = formList.findIndex((e) => e.value === currentStep.value) === formList.length - 1;
            ...
            <Modal open={open} onClose={onClose}>
                <Box sx={style}>
                    <Scrollbar sx={{ maxHeight: '80vh' }}>
                        {currentStep.value === 'anamnesis' && <AnamnesisModal handleNext={handleNext} formData={formData} onClose={onClose} dataSurveilans={dataSurveilans} />}
                        {currentStep.value === 'pemeriksaanFisik' && <PemeriksaanFisikModal handleNext={handleNext} handleBack={handleBack} formData={formData} onClose={onClose} />}
                        {currentStep.value === 'pemilihanForm' && <PemilihanFormModal handleChooseForm={handleChooseForm} handleBack={handleBack} formData={formData} />}
                        {currentStep.value === 'kondisiUmum' && <KondisiUmumModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'kulitBulu' && <KulitBuluModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'membranMukosa' && <MembranMukosaModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'kelenjarLimpa' && <KelenjarLimpaModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'muskulosketal' && <MuskulosketalModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'sistemSirkulasi' && <SistemSirkulasiModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'sistemRespirasi' && <SistemRespirasiModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'sistemDigesti' && <SistemDigestiModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'sistemUrogenital' && <SistemUrogenitalModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'sistemSyaraf' && <SistemSyarafModal handleNext={handleNext} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'mataDanTelinga' && <MataDanTelingaModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
                        {currentStep.value === 'diagnosaSementara' && <DiagnosaSementaraModal handleNext={handleNext} handleBack={handleBack} formData={formData} isLastForm={isLastForm} />}
