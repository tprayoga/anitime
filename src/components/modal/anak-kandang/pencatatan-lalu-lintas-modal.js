import { Autocomplete, Button, Grid, InputAdornment, MenuItem, Modal, TextField, Typography } from "@mui/material";
...
import { useCreateData, useGetData, useGetFulLData, useUpdateData } from "src/api/custom-api";
...
export default function PencatatanLaluLintasModal({
    ...
    const {
        data: dataTujuanPembelian,
        getFullData: getTujuanPembelian
    } = useGetFulLData();
    ...
    const {
        createData: createLaluLintasTernak,
    } = useCreateData();

    const {
        updateData: updateLaluLintasTernak,
    } = useUpdateData();
    ...
    const IMAGE_URL = type === 'EDIT' && selectedData?.fotoSertifikat ? `${FILES_API}/${selectedData?.collectionId}/${selectedData?.id}/${selectedData?.fotoSertifikat}?token=${pb.authStore.token}` : '';
    ...
    useEffect(() => {
        ...
        getBCS('listBCS');
        getTernak(1, 5, `RFID~"${search}" && peternakan = "${user.user.createdBy}"`, '-created', 'ternak', 'kandang')
    ...
    useEffect(() => {
        if (selectedData) {
            reset({
                ...
                kandang: selectedData?.expand?.kandang?.namaKandang,
                bodyConditionalScore: selectedData?.expand?.ternak?.bodyConditionalScore,
                rfid: selectedData.expand.ternak.RFID,
            })
        }
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        const findData = dataTernak.find((e) => e.RFID === data.rfid);
        const body = {
            ternak: findData.id,
            kandang: findData.expand.kandang.id,
    ...
    useEffect(() => {
        if (getValues('rfid')) {
            ...
            if (findData) {
                setValue('bodyConditionalScore', findData.bodyConditionalScore);
    ...
    const modalHeader = (
        <Box sx={{
            ...
            <Typography sx={{ fontWeight: 'bold', fontSize: 'h4.fontSize' }}>Pencatatan Lalu Lintas</Typography>
        </Box>
    )
    const modalBody = (
        <Grid container spacing={1} p={2}>
            ...
            <Grid item xs={12} sm={6}>
                <RHFTextField name="kandang" label="Kandang" sx={{ mt: 1 }} disabled />
            </Grid>
            <Grid item xs={12} sm={6}>
            ...
            <Grid item xs={12} sm={6}>
                <RHFSelect name="bodyConditionalScore" label="Body Conditional Score (BCS)" sx={{ mt: 1 }} disabled>
                    {dataBCS?.map((option) => (
                        <MenuItem value={option.name} key={option.name}>{option.name}</MenuItem>
                    ))}
                </RHFSelect>
            ...
            <Grid item xs={12}>
                <RHFUploadField name="fotoSertifikat" label="Foto Sertifikat Kesehatan Hewan" defaultValue={IMAGE_URL} />
            </Grid>
        </Grid>
    )
    const modalFooter = (
