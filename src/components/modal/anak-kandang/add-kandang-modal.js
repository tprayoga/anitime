...
export default function AddKandangModal({ open, onClose, selectedData, setSelectedData, type, setType, getKandang, setRefetch, ...other }) {
    ...
    const schema = Yup.object().shape({
        ...
        limitKandang: Yup.number()
            .required('Limit Kandang Wajib Diisi')
            .min(1, 'Limit Kandang wajib lebih dari 0'),
    ...
    const modalHeader = (
        <Box sx={{
            ...
            <Typography sx={{ fontWeight: 'bold', fontSize: 'h4.fontSize' }}>Input Data</Typography>
        </Box>
    )

    const modalBody = (
        <Box sx={{ borderRadius: 1, p: 2, }}>
            <RHFTextField name="namaKandang" label="Nama Kandang" sx={{ mt: 1 }} />
            <Stack
                mt={3}
                sx={{
            ...
            >
                <RHFTextField name="luasKandang" label="Luas Kandang" type="number" />
                <RHFSelect
                    name="satuanKandang"
                    label="Satuan Kandang"
                    sx={{
                        width: { xs: '100%', sm: '45%' },
                    }}
            ...
            <RHFTextField name="limitKandang" label="Limit Kandang" sx={{ mt: 3 }} type="number" />
